...
class Conv2Former(nn.Module):
    def __init__(self, kernel_size, img_size=224, in_chans=3, num_classes=1000, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
        ...
        for i in range(len(dims)):
            stage = nn.Sequential(
                *[Block(index=cur + j, dim=dims[i], kernel_size=kernel_size, drop_path=dp_rates[cur + j], num_head=num_heads[i],
                        window_size=window_sizes[i], mlp_ratio=mlp_ratios[i]) for j in range(depths[i])]
            )
