...
class CBAM(nn.Module):
    """
    @misc{woo2018cbamconvolutionalABL_MFBlockattention,
      title={CBAM: Convolutional ABL_MFBlock Attention Module},
...
class ModFormer(nn.Module):
    def __init__(self, in_chans=1, num_classes=6, dims=None, depths=None, kernel_size=4):
        ...
        for i in range(len(dims)):
            ...
            stage = nn.Sequential(
                *[ABL_MFBlock(dims[i], drop_path_prob=0.06) for _ in range(depths[i])],  # 降低drop_path概率
