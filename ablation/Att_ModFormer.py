...
class CBAM(nn.Module):
    """
    @misc{woo2018cbamconvolutionalMFBlockattention,
      title={CBAM: Convolutional MFBlock Attention Module},
...
class ConvModulation(nn.Module):
    ...
    def __init__(self, dim, kernel_size=3):
        ...
        self.cbam_atten = SEModule(channels=dim)
        # CBAM(n_channels=dim, kernel_size=kernel_size)
        # self.se_atten = SEModule(channels=dim)
...
class ModFormer(nn.Module):
    def __init__(self, in_chans=1, num_classes=6, dims=None, depths=None, kernel_size=4):
        ...
        for i in range(len(dims)):
            ...
            stage = nn.Sequential(
                *[MFBlock(dims[i], drop_path_prob=0.06) for _ in range(depths[i])],  # 降低drop_path概率
